...
public class BoardGen : MonoBehaviour
{
  ...
  void Start()
  {
    ...
    if (isVR)
    {
      // Mode VR : Utiliser UI Image avec échelle réduite
      float uiScaleFactor = 0.5f;
      RectTransform rectTransformOpaque = mGameObjectOpaque.AddComponent<RectTransform>();
      rectTransformOpaque.sizeDelta = new Vector2(mBaseSpriteOpaque.texture.width * uiScaleFactor, mBaseSpriteOpaque.texture.height * uiScaleFactor);
      UnityEngine.UI.Image imageOpaque = mGameObjectOpaque.AddComponent<UnityEngine.UI.Image>();
      imageOpaque.sprite = mBaseSpriteOpaque;
      imageOpaque.raycastTarget = false;
    }
    else
    {
      // Mode normal : Utiliser SpriteRenderer
    ...
    if (isVR)
    {
      // Mode VR : Utiliser UI Image avec échelle réduite
      float uiScaleFactor = 0.5f;
      RectTransform rectTransformTransparent = mGameObjectTransparent.AddComponent<RectTransform>();
      rectTransformTransparent.sizeDelta = new Vector2(mBaseSpriteTransparent.texture.width * uiScaleFactor, mBaseSpriteTransparent.texture.height * uiScaleFactor);
      UnityEngine.UI.Image imageTransparent = mGameObjectTransparent.AddComponent<UnityEngine.UI.Image>();
      imageTransparent.sprite = mBaseSpriteTransparent;
      imageTransparent.raycastTarget = false;
    }
    else
    {
      // Mode normal : Utiliser SpriteRenderer
    ...
    // Parenter les objets Opaque et Transparent au même parent que les tuiles si en VR
    if (isVR && parentForTiles != null)
    {
      mGameObjectOpaque.transform.SetParent(parentForTiles, false);
      mGameObjectTransparent.transform.SetParent(parentForTiles, false);
      
      // Position au centre du parent
      if (mGameObjectOpaque.GetComponent<RectTransform>() != null)
      {
        mGameObjectOpaque.GetComponent<RectTransform>().anchoredPosition = Vector2.zero;
      }
      if (mGameObjectTransparent.GetComponent<RectTransform>() != null)
      {
        mGameObjectTransparent.GetComponent<RectTransform>().anchoredPosition = Vector2.zero;
      }
    }
  ...
  {
    // Ne pas déplacer la caméra en VR car cela affecte tout le XR Origin
    // Vérifier si nous ne sommes pas en mode VR avant de déplacer la caméra
  ...
  public static GameObject CreateGameObjectFromTile(Tile tile)
  {
    ...
    // Vérifier si on est en mode VR pour utiliser UI au lieu de SpriteRenderer
    bool isVR = UnityEngine.XR.XRSettings.enabled;
    
    if (isVR)
    {
      // Mode VR : Utiliser des composants UI avec échelle réduite
      // Facteur d'échelle pour adapter les pixels de texture aux unités UI
      float uiScaleFactor = 0.5f; // Réduit les dimensions de 50%
      
      RectTransform rectTransform = obj.AddComponent<RectTransform>();
      rectTransform.anchorMin = new Vector2(0, 0);
      rectTransform.anchorMax = new Vector2(0, 0);
      rectTransform.pivot = new Vector2(0.5f, 0.5f);
      rectTransform.anchoredPosition = new Vector2(tile.xIndex * Tile.tileSize * uiScaleFactor, tile.yIndex * Tile.tileSize * uiScaleFactor);
      rectTransform.sizeDelta = new Vector2((Tile.padding * 2 + Tile.tileSize) * uiScaleFactor, (Tile.padding * 2 + Tile.tileSize) * uiScaleFactor);
